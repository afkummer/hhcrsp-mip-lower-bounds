...
// Defines a callback to stop the search if a certain lower bound is achieved.
ILOMIPCALLBACK1(BestBoundAchieved, double, bbLimit) {
   if (getBestObjValue() >= bbLimit) {
      cout << "WARNING: Best bound greater than the limit.\nStopping the search." << endl;
      abort();
   }
}
...
void MipModel::solve() {
   ...
   //m_cplex.setParam(IloCplex::Param::Emphasis::Numerical, IloTrue);
...
std::vector<std::vector <std::tuple<int,int>>> MipModel::extractRoutes() const {
   std::vector<std::vector <std::tuple<int,int>>> routes(m_inst.numVehicles());
   for (int v = 0; v < m_inst.numVehicles(); ++v)
      routes[v] = extractRoute(v);
   return routes;
}
      ...
      for (int j = 0; j < m_inst.numNodes() - 1; ++j) {
         for (int v = 0; v < m_inst.numVehicles(); ++v) {
            for (int s = 0; s < m_inst.numSkills(); ++s) {
                  ...
                  expr += Solution::COEFS[0] * m_inst.distance(i, j) * x;
            }
         }
      }
   }
   ...
   for (int i = 1; i < m_inst.numNodes() - 1; ++i) {
      for (int v = 0; v < m_inst.numVehicles(); ++v) {
         for (int s = 0; s < m_inst.numSkills(); ++s) {
            ...
            // Original Mankowska et al. (2014) time-windows.
